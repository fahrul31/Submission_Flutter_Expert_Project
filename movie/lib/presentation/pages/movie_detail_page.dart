    ...
    return Scaffold(
      body: BlocBuilder<MovieDetailBloc, MovieDetailState>(
        ...
        if (state is MovieDetailLoading) {
          ...
          return SafeArea(
            child: DetailContent(movieDetail, movieRecommendations),
          );
      ...
      children: [
        BlocListener<MovieDetailWatchlistBloc, MovieDetailWatchlistState>(
          listener: (context, state) {
            if (state is MovieDetailWatchlistSaved) {
              if (state.message != '') {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(state.message!)),
                );
              }
            } else if (state is MovieDetailWatchlistNotSaved) {
              if (state.message != '') {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text(state.message!)),
                );
                                ...
                                return ElevatedButton(
                                  onPressed: () {
                                    if (state is MovieDetailWatchlistSaved) {
                                      context
                                          .read<MovieDetailWatchlistBloc>()
                                          .add(RemoveWatchlistChanged(movie));
                                    } else if (state
                                        is MovieDetailWatchlistNotSaved) {
                                      context
                                          .read<MovieDetailWatchlistBloc>()
                                          .add(SaveWatchlistChanged(movie));
                                    }
                                  },
                                  child: Row(
                                    ...
                                    children: [
                                      BlocBuilder<MovieDetailWatchlistBloc,
                                          ...
                                          builder: (context, state) {
                                        if (state
                                            is MovieDetailWatchlistLoading) {
                                          return const Center(
                                            child: CircularProgressIndicator(),
                                          );
                                        } else if (state
                                            is MovieDetailWatchlistSaved) {
